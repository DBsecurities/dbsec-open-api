"""주식정정주문- NXT거래소 (CSPAT00710) — standalone 예제.
...
실행:
    ...
    python kr_stock_order/kr_stock_order_modify_nxt.py
    # examples/kr_stock_order/ 폴더에서 실행하는 경우:
    python kr_stock_order_modify_nxt.py
...
resp, data = call_rest(
    url="/api/v1/trading/kr-stock/order-revision-nxt",
    body={
        "In": {
            "OrgOrdNo": 340808,  # 원주문번호 (int) - 주식주문 완료시 Out되는 OrdNo 값 입력 (DB증권 거래 시스템에서 채번된 주문번호)
            ...
            "OrdPrc": 4370,  # 주문가 (int) - 정정주문 1주당 주문가격
        },
    },
    label="주식정정주문- NXT거래소",
)
