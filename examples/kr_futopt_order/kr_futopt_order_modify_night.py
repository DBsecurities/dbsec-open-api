"""선물옵션 정정주문 (야간) (CFOHT00200) — standalone 예제.
...
실행:
    ...
    python kr_futopt_order/kr_futopt_order_modify_night.py
    # examples/kr_futopt_order/ 폴더에서 실행하는 경우:
    python kr_futopt_order_modify_night.py
...
resp, data = call_rest(
    url="/api/v1/trading/night-futureoption/order-revision",
    body={
        "In": {
            "FnoIsuNo": "101V3000",  # 선물옵션종목번호 (str)
            "OrgOrdNo": 2,  # 원주문번호 (int)
            "FnoOrdprcPtnCode": "00",  # 선물옵션호가유형코드 (str) - 00:지정가 10:지정가(IOC)
            "OrdPrc": 320.55,  # 주문가격 (int)
            "MdfyQty": 1,  # 정정수량 (int)
        },
    },
    label="선물옵션 정정주문 (야간)",
