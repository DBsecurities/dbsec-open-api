...
실행:
    ...
    python kr_chart/kr_chart_chart_week.py
    # examples/kr_chart/ 폴더에서 실행하는 경우:
    python kr_chart_chart_week.py
...
resp, data = call_rest(
    ...
    body={
        "In": {
            ...
            "InputDate1": "20260519",  # 입력날짜1 (str) - YYYYMMDD (조회 시작일)
            "InputDate2": "20260526",  # 입력날짜2 (str) - YYYYMMDD (조회 종료일)
            "InputOrgAdjPrc": "1",  # 수정주가사용여부 (str) - 0:수정주가 미사용 1: 수정주가 사용
            "InputPeriodDivCode": "W",  # 입력주 (str) - "W"
        },
    },
    label="주차트조회",
)
