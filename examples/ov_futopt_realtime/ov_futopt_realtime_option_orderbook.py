"""[실시간]해외옵션호가 [L02] — standalone WebSocket 예제.
...
해외옵션 실시간 호가 API 입니다. ※ 해외선물옵션 API시세 신청이 되어있지 않은 경우 실시간 시세를 수신 하실 수 없습니다. ※ API시세(유료) 신청방법 GTS(Happy+ Global) : [1761] API시세 신청 ※ 해외선물옵션 가이드는 다음 주소에서 확인 가능하십니다. http://link.dbsec.co.kr/gts/ebook/index.html#page=1 ※ GTS 다운로드는 다음 링크 확인 부탁드리겠습니다. https://www.dbsec.co.kr/research/osft/re_...
...
실행:
    ...
    python ov_futopt_realtime/ov_futopt_realtime_option_orderbook.py
    # examples/ov_futopt_realtime/ 폴더에서 실행하는 경우:
    python ov_futopt_realtime_option_orderbook.py
