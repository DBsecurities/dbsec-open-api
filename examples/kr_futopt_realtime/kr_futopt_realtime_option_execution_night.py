"""[실시간]옵션체결(야간) [O30] — standalone WebSocket 예제.
...
실행:
    ...
    python kr_futopt_realtime/kr_futopt_realtime_option_execution_night.py
    # examples/kr_futopt_realtime/ 폴더에서 실행하는 경우:
    python kr_futopt_realtime_option_execution_night.py
...
── 응답 파라미터 (Out) ─ OUT_BEGIN ──────────────────────
  [TR O30]
    ...
    rho  (문자)  로
    thpr  (문자)  이론가
    IntsVltlprpr  (문자)  내재 변동성(현재가)
    IntsVltlaskp  (문자)  내재 변동성(매도호가)
    IntsVltlbidp  (문자)  내재 변동성(매수호가)
