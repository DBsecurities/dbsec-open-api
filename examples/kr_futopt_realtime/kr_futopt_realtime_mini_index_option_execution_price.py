"""[실시간]미니지수옵션체결가 [O90] — standalone WebSocket 예제.
...
실행:
    ...
    python kr_futopt_realtime/kr_futopt_realtime_mini_index_option_execution_price.py
    # examples/kr_futopt_realtime/ 폴더에서 실행하는 경우:
    python kr_futopt_realtime_mini_index_option_execution_price.py
...
run_ws(ws_subscribe(
    ...
    tr_cd="O90",  # 거래코드 (str) - TR코드입력: O90
    tr_key="",  # 종목코드 (str) - 미니지수옵션: KO ※ 종목분류코드 + 미니지수옵션종목코드 입력
