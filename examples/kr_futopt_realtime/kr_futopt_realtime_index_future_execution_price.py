"""[실시간]지수선물체결가 [F00] — standalone WebSocket 예제.
...
실행:
    ...
    python kr_futopt_realtime/kr_futopt_realtime_index_future_execution_price.py
    # examples/kr_futopt_realtime/ 폴더에서 실행하는 경우:
    python kr_futopt_realtime_index_future_execution_price.py
...
── 응답 파라미터 (Out) ─ OUT_BEGIN ──────────────────────
  [TR F00]
    ...
    AntcNmixclscode  (문자)  예상지수구분코드 — B : 장전 예상체결 0: 정규장 A : 장후 예상체결
...
run_ws(ws_subscribe(
    ...
    tr_cd="F00",  # 거래코드 (str) - TR코드입력: F00
    tr_key="",  # 종목코드 (str) - 지수선물: F ※ 종목분류코드 + 지수선물종목코드 입력 ※ 종목분류코드는 두자리 입력이 필요하므로, F + " " (공백) 문자를 넣어 2바이트를 맞춰 입력 부탁드리겠습니다.
