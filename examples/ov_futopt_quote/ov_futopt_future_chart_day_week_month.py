"""해외선물 일주월차트조회 (pibg7303) — standalone 예제.
...
해외선물 일주월 차트 조회 API 입니다. ※ 해외선물옵션 API시세 신청이 되어있지 않은 경우 시세를 수신 하실 수 없습니다. ※ API시세(유료) 신청방법 GTS(Happy+ Global) : [1761] API시세 신청

토큰은 examples/dbsec_helper.py 가 자동 발급/캐싱합니다.

실행:
    # examples/ 폴더에서 실행하는 경우:
    python ov_futopt_quote/ov_futopt_future_chart_day_week_month.py
    # examples/ov_futopt_quote/ 폴더에서 실행하는 경우:
    python ov_futopt_future_chart_day_week_month.py
...
resp, data = call_rest(
    ...
    body={
        "In": {
            ...
            "Dtgb": "2",  # 데이터구분 (str) - 0: 일(daily) 1: 주(weekly) 2: 월(monthly)
